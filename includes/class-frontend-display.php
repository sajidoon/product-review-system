...
class PRS_Frontend_Display {
    ...
    public function display_product_info($content) {
        ...
        if (!empty($overall_rating)) {
            ...
            if ($editor_choice) {
                $output .= '<div class="editors-choice-badge">' . __('Editor\'s Choice', 'product-review-system') . '</div>';
            }
            $output .= '<div class="overall-score">';
            $output .= '<span class="score-number">' . esc_html($overall_rating) . '</span>';
            $output .= '<span class="score-label">' . __('Overall Score', 'product-review-system') . '</span>';
            $output .= '</div>';
            $output .= '</div>';
        }
        ...
        if (!empty($ratings)) {
            ...
            $output .= '<h3>' . __('Detailed Ratings', 'product-review-system') . '</h3>';
        ...
        if (!empty($pros) || !empty($cons)) {
            ...
            if (!empty($pros)) {
                ...
                $output .= '<h4>✓ ' . __('Pros', 'product-review-system') . '</h4>';
                $output .= '<ul>';
                foreach ($pros as $pro) {
            ...
            if (!empty($cons)) {
                ...
                $output .= '<h4>✗ ' . __('Cons', 'product-review-system') . '</h4>';
                $output .= '<ul>';
                foreach ($cons as $con) {
        ...
        if (!empty($product_prices)) {
            ...
            $output .= get_the_title() . ' - ' . __('Price Comparison', 'product-review-system');
            $output .= '</div>';
            ...
            foreach ($product_prices as $price) {
                if (!empty($price['store']) && !empty($price['price'])) {
                    ...
                    if (!empty($price['url'])) {
                        $output .= '<div class="buy-now"><a href="' . esc_url($price['url']) . '" target="_blank" class="button">' . __('Buy Now', 'product-review-system') . '</a></div>';
                    }
                    $output .= '</div>';
                }
            }
        ...
        if (!empty($features)) {
            ...
            $output .= '<div class="collapsible-header" onclick="toggleCollapsible(this)">';
            $output .= __('Technical Specifications', 'product-review-system');
            $output .= '</div>';
            $output .= '<div class="collapsible-content">';
